...
func (r *Runtime) applyAutoBanSignal(ctx context.Context, signal store.BanSignal) {
	...
	if !r.autoBanEngineAllowed() || !settings.Enabled || (signal.Source == "usage" && !settings.Sources.Usage) || (signal.Source == "inspection" && !settings.Sources.Inspection) {
		return
	}
	if signal.AtMS == 0 {
...
func (r *Runtime) scheduleAutoBan(ctx context.Context) {
	for {
		...
		if !r.autoBanEngineAllowed() || !settings.Enabled {
			continue
		}
