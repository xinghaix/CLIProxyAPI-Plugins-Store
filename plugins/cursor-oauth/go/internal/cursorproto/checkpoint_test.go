	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantAction == "user_message_action" {
				...
				text := userMessage.Get(field(userMessage, "text")).String()
				// The bridge prepends a no-tool system constraint when no tools are declared.
				require.Contains(t, text, "system_constraint")
				require.True(t, strings.HasSuffix(text, tt.wantText), "text should end with the user prompt")
			}
		})
	}
}
