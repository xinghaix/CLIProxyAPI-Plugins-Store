...
func Advance(prev []State, current []usage.Window, now time.Time, skew time.Duration) Result {
...
func fromWindow(prev State, window usage.Window, now time.Time) State {
	state := State{
	...
	case prev.Hypothesis == "fixed":
		state.Phase = PhaseFixed
		state.Hypothesis = "fixed"
	...
	case prev.Phase == PhaseBlocked || prev.Phase == PhaseDue:
		...
		state.SeenBlocked = true
		state.Hypothesis = ""
	case prev.Hypothesis == "anchored" || prev.Phase == PhaseAnchored:
		state.Phase = PhaseAnchored
		state.Hypothesis = "anchored"
...
func windowKey(window usage.Window) string {
